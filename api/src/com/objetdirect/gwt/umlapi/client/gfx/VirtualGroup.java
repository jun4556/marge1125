...
class VirtualGroup extends IncubatorGfxObject {
	...
	public void draw() {
		if (!this.isVisible) {
			Logger.getGlobal().info(this + " is not visible");
			return;
		}
		if (this.canvas == null) {
			Logger.getGlobal().info("canvas is null for " + this);
		}

		Logger.getGlobal().info("Starting drawing " + this);
		for (final IncubatorGfxObject incubatorGfxObject : this.incubatorGfxObjectSet) {
			incubatorGfxObject.draw();
		}
		Logger.getGlobal().info("Ending drawing " + this);
	}
