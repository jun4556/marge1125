...
public class Keyboard {
	...
	public static void push(final char keyCode, final boolean isCtrlDown, final boolean isAltDown, final boolean isShiftDown, final boolean isMetaDown) {
		if (!Keyboard.imEnabled) {
			Logger.getGlobal().info("Keyboard pushed but disabled");
			return;
		}
		Logger.getGlobal().info("Keyboard down Key :" + keyCode + "(" + (int) keyCode + ") with ctrl " + isCtrlDown + " alt " + isAltDown + " shift " + isShiftDown);
		switch (keyCode) {
			case 'X':
				if (isCtrlDown) {
