...
public abstract class UMLArtifact {
	...
	public GfxObject getGfxObject() {
		...
		if (!this.isBuilt) {
			...
			Logger.getGlobal().info("([" + (System.currentTimeMillis() - t) + "ms]) to build " + this);
			this.isBuilt = true;
		}
	...
	public void moveTo(final Point newLocation) {
		if (!this.isALink()) {
			...
			Logger.getGlobal().severe("Can't move a line ! (moveTo called on " + this + ")");
		}
	...
	public void rebuildGfxObject() {
		...
		Logger.getGlobal().info("([" + (System.currentTimeMillis() - t) + "ms]) to build " + this);
		for (final Entry<LinkArtifact, UMLArtifact> dependentUMLArtifact : this.getDependentUMLArtifacts().entrySet()) {
			Logger.getGlobal().info("Rebuilding : " + dependentUMLArtifact);
			new Scheduler.Task("RebuildingDependencyFor" + this) {
				@Override
				public void process() {
					final long t2 = System.currentTimeMillis();
					dependentUMLArtifact.getKey().rebuildGfxObject();
					Logger.getGlobal().info("([" + (System.currentTimeMillis() - t2) + "ms]) to arrow " + this);
				}
			};
		}
		Logger.getGlobal().info("([" + (System.currentTimeMillis() - t) + "ms]) to rebuild " + this + " with dependency");
	...
	public void removeDependency(final LinkArtifact dependentUMLArtifact) {
		Logger.getGlobal().info(this + "removing depency with" + dependentUMLArtifact);
	...
	void addDependency(final LinkArtifact dependentUMLArtifact, final UMLArtifact linkedUMLArtifact) {
		Logger.getGlobal().info(this + "adding depency with" + dependentUMLArtifact + " - " + linkedUMLArtifact);
