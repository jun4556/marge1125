...
public abstract class GeometryPlatform {
	...
	public ArrayList<Point> getLineBetween(final UMLArtifact firstUMLArtifact, final UMLArtifact secondUMLArtifact) {
		...
		Logger.getGlobal().info("([" + (System.currentTimeMillis() - t) + "ms]) to compute line between " + firstUMLArtifact + " and " + secondUMLArtifact);
		return pointList;
	}
	...
	public Point getPointForLine(final UMLArtifact uMLArtifact, final Point point) {
		...
		Logger.getGlobal().info("([" + (System.currentTimeMillis() - t) + "ms]) to compute line between " + uMLArtifact + " and a point");
		return pt;
	}
