...
diff_match_patch.prototype.diff_cleanupSemantic = function(a) {
    ...
    for (f = 1; f < a.length;) {
        if (a[f - 1][0] == DIFF_DELETE && a[f][0] == DIFF_INSERT) {
            ...
            } else if (e >= b.length / 2 || e >= c.length / 2) a.splice(f, 0, new diff_match_patch.Diff(DIFF_EQUAL, b.substring(0, e))), a[f - 1][0] = DIFF_INSERT, a[f - 1][1] = c.substring(0, c.length - e), a[f + 1][0] = DIFF_DELETE, a[f + 1][1] = b.substring(e), f++;
            f++
        }
        f++
    }
...
diff_match_patch.prototype.diff_cleanupSemanticLossless = function(a) {
    ...
    for (var c = 1; c < a.length - 1;) {
        if (a[c - 1][0] == DIFF_EQUAL && a[c + 1][0] == DIFF_EQUAL) {
            ...
            a[c - 1][1] != g && (g ? a[c - 1][1] = g : (a.splice(c - 1, 1), c--), a[c][1] = h, l ? a[c + 1][1] = l : (a.splice(c + 1, 1), c--))
        }
        c++
    }
};
...
diff_match_patch.prototype.diff_cleanupMerge = function(a) {
    ...
    c = !1;
    for (b = 1; b < a.length - 1;) a[b - 1][0] == DIFF_EQUAL && a[b + 1][0] == DIFF_EQUAL && (a[b][1].substring(a[b][1].length - a[b - 1][1].length) == a[b - 1][1] ? (a[b][1] = a[b - 1][1] + a[b][1].substring(0, a[b][1].length - a[b - 1][1].length), a[b + 1][1] = a[b - 1][1] + a[b + 1][1], a.splice(b - 1, 1), c = !0) : a[b][1].substring(0, a[b + 1][1].length) == a[b + 1][1] && (a[b - 1][1] += a[b + 1][1], a[b][1] = a[b][1].substring(a[b + 1][1].length) + a[b + 1][1], a.splice(b + 1, 1), c = !0)), b++;
    ...
    for (var c = [], d = 0, e = 0, f = b.split(/\t/g), g = 0; g < f.length; g++) {
        ...
        switch (f[g].charAt(0)) {
            ...
            case "=":
                ...
                h = a.substring(e, e += l);
                "=" == f[g].charAt(0) ? c[d++] = new diff_match_patch.Diff(DIFF_EQUAL, h) : c[d++] = new diff_match_patch.Diff(DIFF_DELETE, h);
                break;
            default:
...
diff_match_patch.prototype.patch_addContext_ = function(a, b) {
    if (0 != b.length) {
        ...
        (d = b.substring(a.start2 + a.length1, a.start2 + a.length1 + d)) && a.diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, d));
        a.start1 -= c.length;
        a.start2 -= c.length;
    ...
    for (var e = 0, f = 0, g = 0, h = d, l = 0; l < b.length; l++) {
        ...
        switch (k) {
            ...
            case DIFF_DELETE:
                ...
                break;
            case DIFF_EQUAL:
                m.length <= 2 * this.Patch_Margin && e && b.length != l + 1 ? (a.diffs[e++] = b[l], a.length1 += m.length, a.length2 += m.length) : m.length >= 2 * this.Patch_Margin && e && (this.patch_addContext_(a, h), c.push(a), a = new diff_match_patch.patch_obj, e = 0, h = d, f = g)
        }
...
diff_match_patch.prototype.patch_apply = function(a, b) {
    ...
    for (var d = 0, e = [], f = 0; f < a.length; f++) {
        ...
        if (h.length > this.Match_MaxBits) {
            ...
            1 != k && (l = this.match_main(b, h.substring(h.length - this.Match_MaxBits), g + h.length - this.Match_MaxBits), -1 == l || k >= l) && (k = -1)
        ...
        if (-1 == k) e[f] = !1, d -= a[f].length2 - a[f].length1;
            ...
            for (l = 0; l < a[f].diffs.length; l++) {
                ...
                p[0] === DIFF_INSERT ? b = b.substring(0, k + m) + p[1] + b.substring(k + m) : p[0] === DIFF_DELETE && (b = b.substring(0, k + m) + b.substring(k + this.diff_xIndex(g, h + p[1].length)));
        ...
        if (!(a[c].length1 <= b)) {
            ...
            for (var e = d.start1, f = d.start2, g = ""; 0 !== d.diffs.length;) {
                ...
                for (; 0 !== d.diffs.length && h.length1 < b - this.Patch_Margin;) {
                    g = d.diffs[0][0];
                    var k = d.diffs[0][1];
                    g === DIFF_INSERT ? (h.length2 += k.length, f += k.length, h.diffs.push(d.diffs.shift()), l = !1) : g === DIFF_DELETE && 1 == h.diffs.length && h.diffs[0][0] == DIFF_EQUAL && k.length > 2 * b ? (h.length1 += k.length, e += k.length, l = !1, h.diffs.push(new diff_match_patch.Diff(g, k)), d.diffs.shift()) : (k = k.substring(0, b - h.length1 - this.Patch_Margin), h.length1 += k.length, e += k.length, g === DIFF_EQUAL ? (h.length2 += k.length, f += k.length) : l = !1, h.diffs.push(new diff_match_patch.Diff(g, k)), k == d.diffs[0][1] ? d.diffs.shift() : d.diffs[0][1] = d.diffs[0][1].substring(k.length))
                }
